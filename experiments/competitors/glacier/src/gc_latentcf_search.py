...
def main():
    parser = ArgumentParser(
        description="Run this script to evaluate LatentCF method."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        help="Dataset that the experiment is running on.",
    )
    parser.add_argument(
        "--pos",
        type=int,
    ...
    for train_index, test_index in skf.split(X, y_copy):
        ...
        internal_model = InceptionTimeClassifier(n_epochs=20, batch_size=4)
        internal_model.fit(
            X_train_processed_padded, np.argmax(y_train, axis=1)
        )
        classifier = GradientInceptionTimeClassifier(internal_model)
        ...
        acc = balanced_accuracy_score(
            y_true=y_test_classes, y_pred=y_pred_classes
        )
        ...
        logger.info(
            f"1dCNN autoencoder trained, with validation loss: {ae_val_loss}."
        )
        ...
        lr_list = (
            [A.lr_list[0]] if A.lr_list[0] is not None else [0.001, 0.0001]
        )
        ...
        logger.info(
            f"LSTM autoencoder trained, with validation loss: {ae_val_loss2}."
        )
        ...
        lr_list2 = (
            [A.lr_list[1]] if A.lr_list[1] is not None else [0.001, 0.0001]
        )
        ...
        lr_list3 = (
            [A.lr_list[2]] if A.lr_list[2] is not None else [0.001, 0.0001]
        )
