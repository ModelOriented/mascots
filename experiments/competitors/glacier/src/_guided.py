...
from experiments.competitors.glacier.src.LIMESegment.Utils.explanations import (
    LIMESegment,
)
...
class ModifiedLatentCF:
    ...
    def fit(self, model):
        ...
        if self.autoencoder:
            ...
            self.decoder_ = keras.Model(
                inputs=decode_input, outputs=decode_output
            )
            self.encoder_ = keras.Model(
                inputs=encode_input, outputs=encode_output
            )
        else:
            self.decoder_ = None
            self.encoder_ = None
    ...
    def weighted_mae(self, original_sample, cf_sample, step_weights):
        return tf.math.reduce_mean(
            tf.math.multiply(
                tf.math.abs(original_sample - cf_sample), step_weights
            )
        )
    ...
    def compute_loss(
        self, original_sample, z_search, step_weights, target_label
    ):
        loss = tf.zeros(shape=())
        decoded = (
            self.decoder_(z_search)
    ...
    def _transform_sample(self, x, step_weights, pred_label):
        ...
        while (
            ...
            with tf.GradientTape() as tape:
                loss, pred_margin_loss, weighted_steps_loss = (
                    self.compute_loss(x, z, step_weights, target_label)
                )
            it += 1
        ...
        res = (
            z.numpy() if self.autoencoder is None else self.decoder_(z).numpy()
        )
...
def get_global_weights(
    ...
    i = IntervalImportance(
        scoring="accuracy", n_interval=10, random_state=random_state
    )
