...
def LSTMFCNClassifier(n_timesteps, n_features, n_output=2, n_LSTM_cells=8):
    ...
    y = keras.layers.Conv1D(
        128, 8, padding="same", kernel_initializer="he_uniform"
    )(y)
    y = keras.layers.BatchNormalization()(y)
    y = keras.layers.ReLU()(y)

    y = keras.layers.Conv1D(
        256, 5, padding="same", kernel_initializer="he_uniform"
    )(y)
    y = keras.layers.BatchNormalization()(y)
    y = keras.layers.ReLU()(y)

    y = keras.layers.Conv1D(
        128, 3, padding="same", kernel_initializer="he_uniform"
    )(y)
    y = keras.layers.BatchNormalization()(y)
    y = keras.layers.ReLU()(y)
...
class GradientInceptionTimeClassifier(tf.keras.Model):
    def __init__(self, inception_model):
        ...
        for i in range(len(inception_model.classifers_)):
            self.classifiers.append(inception_model.classifers_[i].model_)
    ...
    def call(self, inputs):
        ...
        outs = list(
            map(
                lambda m: m(inputs)[tf.newaxis, :, :],
                self.classifiers,
            )
        )
        outs = tf.stack(outs)
        outs = tf.reduce_sum(outs, axis=0)
        outs = outs / len(self.classifiers)
        return outs[0, :, :]
