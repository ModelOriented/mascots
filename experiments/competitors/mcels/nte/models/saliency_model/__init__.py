...
from experiments.competitors.mcels.nte.utils.perturbation_manager import (
    PerturbationManager,
)
...
class SHAPSaliency(Saliency):
    ...
    def __init__(self, background_data, background_label, predict_fn, args):
        ...
        self.explainer = shap.KernelExplainer(
            model=self.fetch_class, data=self.background_data
        )
        self.args = args
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        if kwargs["save_perturbations"]:
            self.perturbation_manager = PerturbationManager(
                ...
                algo="lime",
                prediction_prob=np.max(kwargs["target"].cpu().data.numpy()),
                original_label=label,
            )
            self.perturbation_manager.update_perturbation(
                self.fetch_class.perturbations,
...
class LimeSaliency(Saliency):
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        if kwargs["save_perturbations"]:
            ...
            self.perturbation_manager.update_perturbation(
                fetch_class.perturbations, confidences=fetch_class.confidences
            )
...
from experiments.competitors.mcels.nte.models.saliency_model.counterfactual_multi_ori import (
    CFExplainer0,
)
