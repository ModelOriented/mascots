...
from experiments.competitors.mcels.fcn_pytorch_model import FCN as fcn
from experiments.competitors.mcels.nte.data.real.multivariate.ArticularyWordRecognition.ArticularyWordRecognition import (
    ArticularyWordRecognitionDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.BasicMotions.BasicMotions import (
    BasicMotionsDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.Cricket.Cricket import (
    CricketDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.Epilepsy.Epilepsy import (
    EpilepsyDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.ERing.ERing import (
    ERingDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.NATOPS.NATOPS import (
    NATOPSDataset,
)
from experiments.competitors.mcels.nte.data.real.multivariate.RacketSports.RacketSports import (
    RacketSportsDataset,
)
...
def get_run_configuration(args, dataset, TASK_ID):
    if args.dataset_type == "train":
        ...
        raise Exception(
            f"Unknown dataset_type : {args.dataset_type}. Supported - [train, test, representative]"
        )
    print(f"Running on {args.dataset_type} data")

    if args.run_mode == "single":
        ds = enumerate(
            zip([data[args.single_sample_id]], [label[args.single_sample_id]])
        )
        ...
        print(
            f"Running in turing mode using slurm tasks jobs{args.jobs_per_task}.samples{args.samples_per_task} . data len{len(data)} TASKID{TASK_ID}..start{int(TASK_ID) * args.samples_per_task}..end{int(TASK_ID) * args.samples_per_task + (args.samples_per_task)}"
        )
        ds = enumerate(
            zip(
                data[
                    int(TASK_ID)
                    * args.samples_per_task : int(TASK_ID)
...
def backgroud_data_configuration(
    BACKGROUND_DATA, BACKGROUND_DATA_PERC, dataset
):
...
def tv_norm(signal, tv_beta):
    ...
    signal_grad = tf.reduce_mean(
        tf.pow(tf.abs(signal[:-1] - signal[1:]), tv_beta)
    )
    return signal_grad
...
def save_timeseries_mul(
    ...
    if enable_wandb:
        wandb.log(
            {"Result": [send_plt_to_wandb(uplt, "1D Saliency Visualization")]}
        )
        ...
        wandb.log(
            {
                "Result": [
                    wandb.Image(
                        np.uint8(255 * perturbated), caption="Perurbation"
                    ),
...
def plot_cmap_mul(data, pertuebated_data, saliency):
    ...
    im = plt.imshow(
        ...
        extent=[
            ...
            float(
                np.min(
                    [np.min(data), np.min(pertuebated_data), np.min(saliency)]
                )
            )
            - 1e-1,
            float(
                np.max(
                    [np.max(data), np.max(pertuebated_data), np.max(saliency)]
                )
            )
            + 1e-1,
        ],
